 ...
 * -8: SH_ERROR_DUPLICATE_PLAYERNAME
 * -99: malloc error
 *
...
int fn_fa_location(char *rest_of_line, game_info_t *gi, struct sockaddr_in them){
    ...
    if (gameId == NULL || pebbleId == NULL || team_name == NULL
        || player_name == NULL || latitude == NULL || longitude == NULL
        || statusReq == NULL){
        free_message_fields(gameId, pebbleId, team_name,
                player_name, latitude, longitude, statusReq, NULL, NULL, NULL);
        return -99;
    }
    ...
    if (strcmp(gameId,"0") == 0){
        ...
        if (team == NULL){
            ...
            #ifdef DEBUG
            printf("Following team registered\n");
            team_print(team);
            #endif
        ...
        if ((status = team_register_fa(gi, team, pebbleId,
                                player_name, them, latitude, longitude)) != 0){
        ...
        // if known, error; free message and return
        else{
            token_delete(token);
            free_message_fields(gameId, pebbleId, team_name,
                player_name, latitude, longitude, statusReq, NULL, NULL, NULL);
    ...
    // if gameId != 0
        ...
        // if correct, update
        if ((status = game_info_validate(gi, gameId, pebbleId,
                            team_name, player_name, latitude, longitude)) != 0){
            free_message_fields(gameId, pebbleId, team_name,
                player_name, latitude, longitude, statusReq, NULL, NULL, NULL);
            token_delete(token);
            return status;
...
int fn_fa_claim(char *rest_of_line, game_info_t *gi, struct sockaddr_in them){
    ...
    if ((status = game_info_validate(gi, gameId, pebbleId, team_name,
                                player_name, latitude, longitude)) != 0){
        free_message_fields(gameId, pebbleId, team_name,
            player_name, latitude, longitude, NULL, kragId, NULL, NULL);
        token_delete(token);
        return status;
...
int fn_ga_status(char *rest_of_line, game_info_t *gi, struct sockaddr_in them){
    ...
    char *player_name = malloc(MESSAGE_LENGTH);
    char *statusReq = malloc(MESSAGE_LENGTH);
    if (gameId == NULL || guideId == NULL || team_name == NULL
        || player_name == NULL || statusReq == NULL){
        
        free_message_fields(gameId, guideId, team_name,
            player_name, NULL, NULL, statusReq, NULL, NULL, NULL);
        return -99;
    }
    // while there are something in the rest of line
    // create the array of the message fields
    ...
    // If gameId != 0
        ...
        // if correct, update, if error return the error value
        if ((status = game_info_validate(gi, gameId, guideId,
                                team_name, player_name, NULL, NULL)) != 0){
            free_message_fields(gameId, guideId, team_name,
                player_name, NULL, NULL, statusReq, NULL, NULL, NULL);
            token_delete(token);
            return status;
...
int fn_ga_hint(char *rest_of_line, game_info_t *gi, struct sockaddr_in them){
    ...
    if (gameId == NULL || guideId == NULL || team_name == NULL
        || player_name == NULL || pebbleId == NULL || hint == NULL){
        
        free_message_fields(gameId, pebbleId, team_name,
            player_name, NULL, NULL, NULL, NULL, guideId, hint);
        return -99;
    }
    // while there are something in the rest of line
    // create the array of the message fields
    ...
    else if (game_info_find_team_pebbleId(gi, pebbleId) != NULL){
        free_message_fields(gameId, pebbleId, team_name,
            player_name, NULL, NULL, NULL, NULL, guideId, hint);
        token_delete(token);
        return 2;
...
token_next(token_t *token){
    ...
    while (i < strlen(token->rest_of_line)){
        ...
        // move on to right hand side when '=' appears
            ...
            // add to token left or right, depending on state
            if (state == 0){
    ...
    // when end of line or creating token, terminate
...
/* free the message fields */
static void
free_message_fields(char *gameId, char *pebbleId, char *team_name,
        char *player_name, char *latitude, char *longitude, char *statusReq,
        char *kragId, char *guideId, char *hint){
